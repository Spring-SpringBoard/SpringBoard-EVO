...
local COMM_VALUE = UnitDefNames.ecommander.metalCost or 150
...
local function IsPlayMode()
	...
	return sb_gameMode == nil or sb_gameMode == "play"
end
--------------------------------------------------------------------------------
-- local funcs
--------------------------------------------------------------------------------
...
local function ProcessLastAlly()
	...
	elseif #activeAllies < 2 then
		...
		for i=1, #allylist do
			local a = allylist[i]
			if (a ~= lastActive)and(a ~= gaiaAllyTeamID) and IsPlayMode() then
				DestroyAlliance(a)
			end
		end
	end
