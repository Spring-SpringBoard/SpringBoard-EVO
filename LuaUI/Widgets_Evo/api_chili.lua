...
local DIR = GetDirectory(source) or ((LUA_DIRNAME or LUAUI_DIRNAME) .."Widgets_Evo/")
CHILI_DIRNAME = DIR .. "chili/"

--------------------------------------------------------------------------------
--------------------------------------------------------------------------------

function widget:GetSBViewSizes(vsx, vsy)
	--0.5 Find a way to limit chili (or entire UI) to just one part of the screen
	-- may require more magix
	local W_RATIO = 1
	local H_RATIO = 1
	return vsx * W_RATIO, vsy * H_RATIO
end
...
function widget:Initialize()
	...
	screen0:Resize(self:GetSBViewSizes(gl.GetViewSizes()))
...
function widget:DrawScreen()
	...
	if (not screen0:IsEmpty()) then
		gl.PushMatrix()
			...
			vsx, vsy = self:GetSBViewSizes(vsx, vsy)
